...
def main():
    ...
    ax.bar(df['Year'], df['Risk Assets (JPY)'], label='Risk Assets', color='lightblue')
    ax.bar(df['Year'], df['Safe Assets (JPY)'], bottom=df['Risk Assets (JPY)'],
           label='Safe Assets', color='lightgreen')
    
    # Format y-axis to billions
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/100000000:.1f}B'))
